...
for(technique in files) {
  ...
  #Loop over all measurements
    ...
    for(i in 1:10) {
      ...
      if(warn == FALSE) {
        file_name = sprintf("/Users/NicolasCardozo/Documents/workspace-all/TassPaper/Subjective-C2/runs_%s/%d/test%d.csv", technique, measurements[index], i)
        warn <- tryCatch({
...
legend("topleft", legend = c("ifs", "strategy", "subjective-c"), col=1:3, pch=c(15:17)) # optional legend
