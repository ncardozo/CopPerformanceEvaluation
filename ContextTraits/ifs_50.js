...
function runs(run) {
    var fs = require("fs");
    var times;
    fs.writeFileSync("./runs_ifs/50/test"+run+".csv", "");
    fs.appendFileSync("./runs_ifs/50/test"+run+".csv", "iteration, Elapsed time in milliseconds \n");
    for(times = 1; times <= 100; times++) {
        var elapsed_time = testContexts();
        fs.appendFileSync("./runs_ifs/50/test"+run+".csv", times + "," + elapsed_time + "\n");
    }
}
