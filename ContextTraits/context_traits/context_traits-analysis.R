...
for(technique in files) {
  ...
  #Loop over all measurements
    ...
    for(i in 1:10) {
      ...
      if(warn == FALSE) {
        file_name = sprintf("/Users/NicolasCardozo/Documents/workspace-all/TassPaper/ContextTraits/runs_%s/%d/test%d.csv", technique, measurements[index], i)
        warn <- tryCatch({
...
legend("topleft", legend = c("ifs", "strategy", "context traits"), col=1:3, pch=c(15:17)) # optional legend
# add a title and subtitle 
title("Context Traits", " ")


matplot(measurements[1:6], total_means[1:6,], type = "b", lty=1, pch=c(15:17), col=1:3, xlab="No. of contexts", ylab="time (in millisecods)") #plot
legend("topleft", legend = c("ifs", "strategy", "context traits"), col=1:3, pch=c(15:17)) # optional legend
# add a title and subtitle 
title("Context Traits", "Zoomed")
